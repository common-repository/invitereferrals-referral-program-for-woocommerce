<?php
/**
* Plugin Name: Customer Referral Program For WooCommerce
	...
	if(!class_exists('Invitereferrals_wc')) { 
		...
		class Invitereferrals_wc
		{
			...
			public function __construct($id) {
				...
				$this->options = array('Invitereferrals_Secret_key' => '', 'Invitereferrals_enable' => 'on' );
			...
			public function Invitereferrals_campaign_script() {
				...
				if($this->options['Invitereferrals_enable'] == 'on' && $this->options['Invitereferrals_key'] != '')
				{
					...
					*/
							...
							$email = $current_user->data->user_email;// the user email id
							$name = $current_user->data->display_name;//first name of customer	
							$rt = time(); 
							$sso_key = md5($this->options['Invitereferrals_Secret_key'].'||'.$email.'||'.$rt);
					...
					if($flag)
					{
						...
						*/
						...
						//echo "<img style='position:absolute; visibility:hidden' src='https://www.ref-r.com/campaign/t1/settings?bid_e=".$this->options['Invitereferrals_key']."&bid=".$this->options['Invitereferrals_id']."&t=420&event=sale&email=$order_email&orderID=$order_id&purchaseValue=$order_total' />";
						echo "<script>ir('track',{ orderID: '".$order_id."', event: 'sale', email: '".$order_email."', fname: '".$order_name."', mobile:'".$order_mobile."', purchaseValue: '".$order_total."'});</script>";
